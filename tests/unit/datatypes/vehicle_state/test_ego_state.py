...
class TestEgoStateSE2:
    ...
    def test_from_rear_axle(self):
        ...
        assert ego_state.ego_metadata == self.ego_metadata
    ...
    def test_center_property(self):
        ...
        assert center.x == pytest.approx(self.ego_metadata.rear_axle_to_center_longitudinal)
    ...
    def test_bounding_box_property(self):
        ...
        bbox_center = BoundingBoxSE2(ego_state.center_se2, self.ego_metadata.length, self.ego_metadata.width)
        assert bbox is not None
        assert bbox.length == self.ego_metadata.length
        assert bbox.width == self.ego_metadata.width
...
class TestEgoStateSE3:
    ...
    def test_from_imu_with_offset(self):
        ...
        ego_state = EgoStateSE3.from_imu(
            imu_se3=imu_pose,
            ego_metadata=ego_metadata_with_offset,
            timestamp=self.timestamp,
        )
    ...
    def test_from_imu_with_rotation(self):
        ...
        assert ego_state.ego_metadata == ego_metadata_with_offset
        assert ego_state.timestamp == self.timestamp
    ...
    def test_bounding_box_properties(self):
        ...
        assert bbox_se3.length == self.ego_metadata.length
        assert bbox_se3.width == self.ego_metadata.width
        assert bbox_se3.height == self.ego_metadata.height
    ...
    def test_ego_state_se2_projection(self):
        ...
        assert ego_state_se2.ego_metadata == self.ego_metadata
