...
from py123d.conversion.abstract_dataset_converter import AbstractDataConverter, DatasetConverterConfig
from py123d.conversion.utils.map_utils.opendrive.opendrive_map_conversion import convert_from_xodr
from py123d.conversion.utils.sensor.lidar_index_registry import CARLALidarIndex
from py123d.datatypes.maps.abstract_map import AbstractMap, MapLayer
...
from py123d.datatypes.sensors.lidar.lidar import LiDARMetadata, LiDARType, lidar_metadata_dict_to_json
from py123d.datatypes.vehicle_state.ego_state import EgoStateSE3Index
from py123d.datatypes.vehicle_state.vehicle_parameters import get_carla_lincoln_mkz_2020_parameters
from py123d.geometry import BoundingBoxSE3Index, Point2D, Point3D, Vector3DIndex
...
class CarlaDataConverter(AbstractDataConverter):
    ...
    def convert_logs(self, worker: WorkerPool) -> None:
        ...
        worker_map(
            worker,
            partial(convert_carla_log_to_arrow, dataset_converter_config=self.dataset_converter_config),
            log_args,
        )
...
def convert_carla_log_to_arrow(
    ...
    def convert_log_internal(args: List[Dict[str, Union[List[str], List[Path]]]]) -> None:
        for log_info in args:
            ...
            output_path: Path = dataset_converter_config.output_path
    ...
    with pa.OSFile(str(log_file_path), "wb") as sink:
        with pa.ipc.new_file(sink, recording_schema) as writer:
            for box_path in bounding_box_paths:
                ...
                if dataset_converter_config.lidar_store_option is not None:
                    lidar_data_dict = _extract_lidar(log_name, sample_name, dataset_converter_config)
                    for lidar_type, lidar_data in lidar_data_dict.items():
                        row_data[lidar_type.serialize()] = [lidar_data]

                if dataset_converter_config.camera_store_option is not None:
                    camera_data_dict = _extract_cameras(data, log_name, sample_name, dataset_converter_config)
...
def _extract_cameras(
    data: Dict[str, Any], log_name: str, sample_name: str, dataset_converter_config: DatasetConverterConfig
...
def _extract_lidar(
    log_name: str, sample_name: str, dataset_converter_config: DatasetConverterConfig
