...
from d123.conversion.utils.log_writer.arrow_ipc_writer import ArrowLogWriter
from d123.conversion.utils.sensor.camera_conventions import CameraConvention, convert_camera_convention
from d123.conversion.utils.sensor.lidar_index_registry import WopdLidarIndex
from d123.conversion.wopd.waymo_map_utils.wopd_map_utils import convert_wopd_map
from d123.conversion.wopd.wopd_utils import parse_range_image_and_camera_projection
...
class WOPDDataConverter(AbstractDataConverter):
    def __init__(
        self,
        splits: List[str],
    ...
    def convert_maps(self, worker: WorkerPool) -> None:
        ...
        worker_map(
            worker,
            partial(convert_wopd_tfrecord_map_to_gpkg, dataset_converter_config=self.dataset_converter_config),
            log_args,
        )
    ...
    def convert_logs(self, worker: WorkerPool) -> None:
        ...
        worker_map(
            worker,
            partial(convert_wopd_tfrecord_log_to_arrow, dataset_converter_config=self.dataset_converter_config),
            log_args,
        )


def convert_wopd_tfrecord_map_to_gpkg(
    args: List[Dict[str, Union[List[str], List[Path]]]], dataset_converter_config: DatasetConverterConfig
) -> List[Any]:
    ...
    for log_info in args:
        ...
        if dataset_converter_config.force_map_conversion or not map_file_path.exists():
...
def convert_wopd_tfrecord_log_to_arrow(
    ...
    for log_info in args:
        try:
            ...
            log_name = str(initial_frame.context.name)
            log_file_path = dataset_converter_config.output_path / split / f"{log_name}.arrow"
            ...
            if dataset_converter_config.force_log_conversion or not log_file_path.exists():
                ...
                log_metadata = LogMetadata(
                    ...
                    camera_metadata=get_wopd_camera_metadata(initial_frame, dataset_converter_config),
                    lidar_metadata=get_wopd_lidar_metadata(initial_frame, dataset_converter_config),
                    map_has_z=True,
                    map_is_local=True,
                )
                ...
                log_writer = ArrowLogWriter(
                    ...
                    dataset_converter_config=dataset_converter_config,
                    log_metadata=log_metadata,
                )

                _write_recording_table(dataset, log_writer, tf_record_path, dataset_converter_config)
...
def _write_recording_table(
    ...
    for frame_idx, data in enumerate(dataset):
        ...
        log_writer.add_row(
            ...
            lidars=_extract_wopd_lidars(frame, dataset_converter_config),
...
def _extract_wopd_cameras(
    ...
    assert dataset_converter_config.camera_store_option == "binary", "Camera store option must be 'binary' for WOPD."
