...
def build_sensor_dataframe(raw_log_path: Path) -> pd.DataFrame:

    # https://github.com/argoverse/av2-api/blob/main/src/av2/datasets/sensor/sensor_dataloader.py#L209

    split = raw_log_path.parent.name
    log_id = raw_log_path.name

    lidar_path = raw_log_path / "sensors" / "lidar"
    cameras_path = raw_log_path / "sensors" / "cameras"
    ...
    for camera_folder in cameras_path.iterdir():
        assert camera_folder.name in AV2_SENSOR_CAMERA_NAMES
