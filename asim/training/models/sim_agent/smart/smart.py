...
from asim.training.models.sim_agent.smart.metrics.min_fde import minFDE
...
class SMART(pl.LightningModule):
    ...
    def validation_step(self, data, batch_idx):
        ...
        self.log(
            "val_cls_acc", self.TokenCls, prog_bar=True, on_step=False, on_epoch=True, batch_size=1, sync_dist=True
        )
    ...
    def match_token_map(self, data):
        ...
        if self.noise:
            topk_indices = torch.argsort(
                torch.sum((token_sample_pt[None] - traj_pos_local.unsqueeze(1)) ** 2, dim=(-2, -1)), dim=1
            )[:, :8]
            sample_topk = torch.randint(
                0, topk_indices.shape[-1], size=(topk_indices.shape[0], 1), device=topk_indices.device
            )
        ...
        token_src_world = (
            torch.bmm(
                token_src[None, ...].repeat(pl_num, 1, 1, 1).reshape(pl_num, -1, 2), rot_mat.view(-1, 2, 2)
        ...
        count_nums = []
        for pl_ in pl_idx_full.unique():
            pt = token2pl[0, token2pl[1, :] == pl_]
