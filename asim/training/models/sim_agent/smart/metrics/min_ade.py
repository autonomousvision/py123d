...
from asim.training.models.sim_agent.smart.metrics.utils import topk, valid_filter
...
class minMultiADE(Metric):
    ...
    def update(
        ...
        if min_criterion == "FDE":
            ...
            self.sum += (
                (torch.norm(pred_topk[torch.arange(pred.size(0)), inds_best] - target, p=2, dim=-1) * valid_mask).sum(
                    dim=-1
                )
            ...
            self.sum += (
                (torch.norm(pred_topk - target.unsqueeze(1), p=2, dim=-1) * valid_mask.unsqueeze(1))
                .sum(dim=-1)
                .min(dim=-1)[0]
                / valid_mask.sum(dim=-1)
            ).sum()
...
class minADE(Metric):
    ...
    def update(
        ...
        self.sum += (
            (
                torch.norm(pred[:, :eval_timestep] - target[:, :eval_timestep], p=2, dim=-1)
                * valid_mask[:, :eval_timestep]
            ).sum(dim=-1)
            / pred.shape[1]
        ).sum()
