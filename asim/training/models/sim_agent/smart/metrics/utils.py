            ...
            if joint:
                if ptr is None:
                    inds_topk = torch.topk(
                        (prob / prob.sum(dim=-1, keepdim=True)).mean(dim=0, keepdim=True),
                        k=max_guesses,
                        dim=-1,
                        largest=True,
                        sorted=True,
                    ...
                    inds_topk = torch.topk(
                        segment_csr(src=prob / prob.sum(dim=-1, keepdim=True), indptr=ptr, reduce="mean"),
                        k=max_guesses,
                        dim=-1,
                        largest=True,
                        sorted=True,
...
def batch_nms(pred_trajs, pred_scores, dist_thresh, num_ret_modes=6, mode="static", speed=None):
    """
...
def batch_nms_token(pred_trajs, pred_scores, dist_thresh, num_ret_modes=6, mode="static", speed=None):
    """
    Args:
        pred_trajs (batch_size, num_modes, num_timestamps, 7)
        pred_scores (batch_size, num_modes):
