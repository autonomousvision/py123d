...
from asim.training.models.sim_agent.smart_forecasting.smart_forecasting_config import SMARTForecastingConfig


class SMARTDecoder(nn.Module):
    def __init__(self, model_config: SMARTForecastingConfig, n_token_agent: int) -> None:
        super(SMARTDecoder, self).__init__()
        self.map_encoder: SMARTMapDecoder = SMARTMapDecoder(model_config)
        self.agent_encoder: TrajectoryAgentDecoder = TrajectoryAgentDecoder(model_config, n_token_agent=n_token_agent)
...
class SMARTMapDecoder(nn.Module):
    def __init__(self, model_config: SMARTForecastingConfig) -> None:
...
class TrajectoryAgentDecoder(nn.Module):
    def __init__(self, model_config: SMARTForecastingConfig, n_token_agent: int) -> None:
        super(TrajectoryAgentDecoder, self).__init__()
        self.model_config = model_config
        ...
        # self.trajectory_predict_head = MLPLayer(
        #     input_dim=model_config.hidden_dim, hidden_dim=model_config.hidden_dim, output_dim=n_token_agent
        # )
        self.apply(weight_init)
