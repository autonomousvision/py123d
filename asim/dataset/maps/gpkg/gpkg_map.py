...
from asim.dataset.maps.abstract_map_objects import AbstractLane, AbstractMapObject
...
class GPKGMap(AbstractMap):
    ...
    def __init__(self, file_path: Path) -> None:
        ...
        self._map_object_getter: Dict[MapObjectType, Callable[[str], Optional[AbstractMapObject]]] = {
            MapObjectType.LANE: self._get_lane,
        }

        # loaded during `.initialize()`
        self._gpd_dataframes: Dict[MapObjectType, gpd.GeoDataFrame] = {}
    ...
    def initialize(self) -> None:
        ...
        for map_layer in list(MapObjectType):
    ...
    def _assert_layer_available(self, layer: MapObjectType) -> None:
        "Checks if layer is available."
        assert layer in self.get_available_map_objects(), f"GPKGMap: MapObjectType {layer.name} is unavailable."
    ...
    def get_proximal_map_objects(
        self, point_2d: Point2D, radius: float, layers: List[MapObjectType]
        ...
        object_map: Dict[MapObjectType, List[AbstractMapObject]] = defaultdict(list)
    ...
    def _get_proximity_map_object(self, patch: geom.Polygon, layer: MapObjectType) -> List[AbstractMapObject]:
        """
    ...
    def _get_lane(self, id: str) -> Optional[AbstractLane]:
        return (
            GPKGLane(id, self._gpd_dataframes[MapObjectType.LANE])
            if id in self._gpd_dataframes[MapObjectType.LANE]["id"].tolist()
            else None
        )
