...
class ViserViewer:
    ...
    def __init__(
        ...
        self._follow_prev_iteration: Optional[int] = None
    ...
    def _run_scene(self, scene: SceneAPI) -> None:
        ...
        self._follow_prev_iteration = None
    ...
    def _apply_ego_follow(self, scene: SceneAPI, iteration: int, follow_enabled: bool) -> None:
        """Translate all client cameras by the ego position delta between timesteps.

        Pure translation: zoom, pan offset, and orientation chosen by the user are
        preserved (the viser position setter moves the look-at point along).
        """
        previous = self._follow_prev_iteration
        self._follow_prev_iteration = iteration
        if not follow_enabled or previous is None or previous == iteration:
            return
        previous_state = scene.get_ego_state_se3_at_iteration(previous)
        current_state = scene.get_ego_state_se3_at_iteration(iteration)
        if previous_state is None or current_state is None:
            return
        # The scene-center offset used for display cancels out in the delta.
        delta = current_state.center_se3.point_3d.array - previous_state.center_se3.point_3d.array
        if not np.any(delta):
            return
